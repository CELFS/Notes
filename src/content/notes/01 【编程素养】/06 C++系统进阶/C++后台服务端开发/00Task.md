...
![image-20240530200519467](http://img.celfs.site/typora/2025/11/22/1763818513969-image-20240530200519467.png)
...
![image-20240531164139172](http://img.celfs.site/typora/2025/11/22/1763818513810-image-20240531164139172.png)
...
* ~~为何程序能识别已经有打开的进程，但无法 kill 呢？难道因为查到的是共享内存中的东西？~~ 
    ...
    * 不调试不知道，这样操作之后，才能更清晰地理解 “共享内存” 的行为和作用（真切理解了在共享内存中的进程与在 shell 能识别的进程的差别），**这种调试收获很大，必须重视那些看起来不起眼的问题**，动手实践才有收获。
    ...
    ![](http://img.celfs.site/typora/2025/11/22/1763818513730-image-20240531222759218.png)
...
* BUG
    ...
    * ~~在打印共享内存的进程信息时，没有显示内容。而如果 `pid == 0`，则有内容。会不会是共享内存的问题？~~ 
    ...
    ![image-20240531221607105](http://img.celfs.site/typora/2025/11/22/1763818513665-image-20240531221607105.png)
...
![image-20240601235450472](http://img.celfs.site/typora/2025/11/22/1763818513612-image-20240601235450472.png)

* 1）进程正常退出（终止了一个进程）

![image-20240601235616489](http://img.celfs.site/typora/2025/11/22/1763818513323-image-20240601235616489.png)

* 2）进程未超时 VS 进程超时

![image-20240602003500010](http://img.celfs.site/typora/2025/11/22/1763818513288-image-20240602003500010.png)

* 3）处理共享内存残留信息

![image-20240602033301941](http://img.celfs.site/typora/2025/11/22/1763818512900-image-20240602033301941.png)

* 4）守护进程仅完成部分任务后被清理

![image-20240602040014676](http://img.celfs.site/typora/2025/11/22/1763818512847-image-20240602040014676.png)

* 守护进程正确处理（demo02 未忽略信号15）

![image-20240602041330863](http://img.celfs.site/typora/2025/11/22/1763818512748-image-20240602041330863.png)

* 守护进程正确处理（demo02 忽略信号15）

![image-20240602041641378](http://img.celfs.site/typora/2025/11/22/1763818512700-image-20240602041641378.png)
...
![image-20240603165739507](http://img.celfs.site/typora/2025/11/22/1763818512619-image-20240603165739507.png)

* 脚本启动所有服务程序

![image-20240603174831686](http://img.celfs.site/typora/2025/11/22/1763818512589-image-20240603174831686.png)
...
![image-20240604183756142](http://img.celfs.site/typora/2025/11/22/1763818512526-image-20240604183756142.png)
...
* 疑问
  ...
  * ~~为什么 `sh stop.sh` 的第一条内容是 “不允许的操作”？这是对守护进程（910）而言的，即前面的  `checkproc` 进程。~~ 
    ...
    ![image-20240604183915069](http://img.celfs.site/typora/2025/11/22/1763818512494-image-20240604183915069.png)
...
* 配置过程
  ...
  * 出现 `Relabeling` 信息
  ...
  ![image-20240604223410707](http://img.celfs.site/typora/2025/11/22/1763818512330-image-20240604223410707.png)

  * 初次配置 `selinux`，重启后

  ![image-20240604223518509](http://img.celfs.site/typora/2025/11/22/1763818512293-image-20240604223518509.png)

  * 配置 `/etc/selinux/config` 
    * `permissive` --> `disabled` 

  ![image-20240604223827647](http://img.celfs.site/typora/2025/11/22/1763818512015-image-20240604223827647.png)

  * 3）指定数据端口范围 `vim /etc/vsftpd.conf` 

  ![image-20240604224636804](http://img.celfs.site/typora/2025/11/22/1763818511724-image-20240604224636804.png)
  ...
  * 报错（需要使用 `sudo` 操作）
  ...
  ![image-20240604225427004](http://img.celfs.site/typora/2025/11/22/1763818511220-image-20240604225427004.png)

  * 执行完毕

  ![image-20240604225535558](http://img.celfs.site/typora/2025/11/22/1763818511146-image-20240604225535558.png)
  ...
  * `start` --> `status` （至此，本地服务器配置完成）
  ...
  ![image-20240604230431718](http://img.celfs.site/typora/2025/11/22/1763818511086-image-20240604230431718.png)

  * 配置云服务访问策略（阿里云为例）
    * 网络和安全组 --> 安全组配置 --> 配置规则 --> 手动添加（如下）

  ![image-20240604230849633](http://img.celfs.site/typora/2025/11/22/1763818510996-image-20240604230849633.png)
...
* 进一步介绍（ftp 传输模式）
  ...
  * 主动模式（port 模式）
  ...
  ![image-20240604231848174](http://img.celfs.site/typora/2025/11/22/1763818510929-image-20240604231848174.png)

  * 被动模式（pasv 模式，默认 / 缺省）
    * 使用高端口

  ![image-20240604231900952](http://img.celfs.site/typora/2025/11/22/1763818510618-image-20240604231900952.png)
...
* 代码流程
  ...
  * 登录服务器
    ...
    * 可输入虚拟机IP --> 提示 Linux 用户名 + 密码 --> 登录（三种方式如下）
    ...
    ![image-20240605194103282](http://img.celfs.site/typora/2025/11/22/1763818510536-image-20240605194103282.png)

    ![image-20240605194810999](http://img.celfs.site/typora/2025/11/22/1763818510492-image-20240605194810999.png)

    ![image-20240605195031523](http://img.celfs.site/typora/2025/11/22/1763818510385-image-20240605195031523.png)
...
![image-20240606181229135](http://img.celfs.site/typora/2025/11/22/1763818510308-image-20240606181229135.png)
...
* Windows 使用 ftp 访问虚拟机
  ...
  * 登录成功，可以看到登录在虚拟机的 `/home/celfs` 路径
  ...
  ![image-20240606181933931](http://img.celfs.site/typora/2025/11/22/1763818510299-image-20240606181933931.png)

  * 测试文件操作权限1（主机 --> 拖动文件 --> 虚拟机 | 权限不足 | 已解决）

  ![image-20240606182254165](http://img.celfs.site/typora/2025/11/22/1763818510221-image-20240606182254165.png)

  * 测试文件操作权限2（虚拟机--> 拖动文件 --> 主机 | 权限满足）

  ![image-20240606182439282](http://img.celfs.site/typora/2025/11/22/1763818510020-image-20240606182439282.png)
  ...
  * **问题解决**（修改写入权限即可 | 默认注释，即关闭写）
  ...
  ![image-20240606185004318](http://img.celfs.site/typora/2025/11/22/1763818509512-image-20240606185004318.png)

  * 写入成功

  ![image-20240606185310951](http://img.celfs.site/typora/2025/11/22/1763818508058-image-20240606185310951.png)

  * Windows 命令行模式 ftp 连接

  ![image-20240606192038149](http://img.celfs.site/typora/2025/11/22/1763818507917-image-20240606192038149.png)

  * 从服务端下载文件到主机

  ![image-20240606192403133](http://img.celfs.site/typora/2025/11/22/1763818507815-image-20240606192403133.png)

  * 从主机本地目录上传文件到服务端

  ![image-20240606200519630](http://img.celfs.site/typora/2025/11/22/1763818507805-image-20240606200519630.png)
...
* 调试
  ...
  * `demo50.cpp` （42 Line）
  ...
  ![image-20240607190154541](http://img.celfs.site/typora/2025/11/22/1763818507737-image-20240607190154541.png)

  * `demo51.cpp` （33 Line）
    * 修改登录信息 --> 修改路径 --> 成功切换目录 + 上传文件

  ![image-20240607190707524](http://img.celfs.site/typora/2025/11/22/1763818507652-image-20240607190707524.png)

  * `demo52.cpp` （37 Line）
    * 修改登录信息 --> 修改路径 --> 成功下载文件

  ![image-20240607192230510](http://img.celfs.site/typora/2025/11/22/1763818507617-image-20240607192230510.png)
  ...
  * 分步实现 --> 列出具体流程 --> 实现子流程
    ...
    * 1）从服务器某个目录下载文件，可指定文件匹配的规则
      ...
      * 1.2）解析 `xml`，得到程序运行的参数
      ...
      ![image-20240608200131606](http://img.celfs.site/typora/2025/11/22/1763818507440-image-20240608200131606.png)
...
* 代码流程
  ...
  * 调试
    ...
    * `<ptype>2</ptype>` 验证三个目录的文件 --> 成功（删除了 `json` 以外的文件）
    ...
    ![image-20240609170635911](http://img.celfs.site/typora/2025/11/22/1763818507046-image-20240609170635911.png)

    * `<ptype>3</ptype>` 
      * 清除日志 --> 删除相关目录 --> 执行
      * ~~区别：不会在服务端自动创建目录，因此需手动创建。（本机测试没有出现这个问题，可能是因为 `celfs` 普通用户在 `sudo` 组）~~ 错了，只是因为 `ptype` 没有改成 3，因此，需要手动创建目录。

    ![image-20240609171701197](http://img.celfs.site/typora/2025/11/22/1763818506592-image-20240609171701197.png)
    ...
    * 成功取走 `csv` 及 `xml` 文件，成功备份目录
    ...
    ![image-20240609172142090](http://img.celfs.site/typora/2025/11/22/1763818506542-image-20240609172142090.png)
    ...
    * **两种报错验证方法** 
    ...
    ![image-20240609172959090](http://img.celfs.site/typora/2025/11/22/1763818506542-image-20240609172959090.png)

    * 显示一致

    ![image-20240609173254821](http://img.celfs.site/typora/2025/11/22/1763818506543-image-20240609173254821.png)
...
* 代码流程
  ...
  * 思路
    ...
    * 两个目录 `client` 和 `server`，其中文件均从服务端下载
    ...
    ![image-20240609222952066](http://img.celfs.site/typora/2025/11/22/1763818506540-image-20240609222952066.png)
    ...
    * 设计 4 个容器（存储文件名 + 文件时间）
    ...
    ![image-20240607203810120](http://img.celfs.site/typora/2025/11/22/1763818506544-image-20240607203810120.png)
...
* BUG
  ...
  * Linux 粘贴指令，总是出现 prompt 消失的现象。
  ...
  ![image-20240611155740411](http://img.celfs.site/typora/2025/11/22/1763818506541-image-20240611155740411.png)
  ...
  * ~~文件成功下载，但**修改时间不统一**；文件时间一侧的**编号也不一致**。~~ 
  ...
  ![image-20240611161144629](http://img.celfs.site/typora/2025/11/22/1763818506538-image-20240611161144629.png)
...
* 代码流程
  ...
  * 容器分析
  ...
  ![image-20240611232444662](http://img.celfs.site/typora/2025/11/22/1763818506505-image-20240611232444662.png)
...
* 代码分析
  ...
  * 调试
  ...
  ![image-20240614211247268](http://img.celfs.site/typora/2025/11/22/1763818506500-image-20240614211247268.png)
  
  ![image-20240614211235941](http://img.celfs.site/typora/2025/11/22/1763818506499-image-20240614211235941.png)
...
* 概括
  * 短连接
  ...
  ![image-20240616172857955](http://img.celfs.site/typora/2025/11/22/1763818506483-image-20240616172857955.png)
  ...
  * 长连接
  ...
  ![image-20240616173002123](http://img.celfs.site/typora/2025/11/22/1763818506481-image-20240616173002123.png)
...
* 代码流程（上传文件请求发送，收到服务端确认报文）
  ...
  ![image-20240616173305854](http://img.celfs.site/typora/2025/11/22/1763818506501-image-20240616173305854.png)
...
* 概括
  * 同步通讯
  ...
  ![image-20240618121218816](http://img.celfs.site/typora/2025/11/22/1763818506409-image-20240618121218816.png)
  ...
  * 异步通讯
  ...
  ![image-20240618121321093](http://img.celfs.site/typora/2025/11/22/1763818506411-image-20240618121321093.png)
